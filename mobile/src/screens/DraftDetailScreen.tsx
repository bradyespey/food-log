...
export default function DraftDetailScreen({ route, navigation }: Props) {
  ...
  const canAnalyze = photos.length > 0 || !!(draft.brand || draft.note);
  ...
  return (
    <View style={s.root}>
      <SafeAreaView edges={['top']} style={s.headerWrap}>
        <View style={s.header}>
          <TouchableOpacity style={s.backBtn} onPress={() => navigation.navigate('Capture', { draftId })}>
      ...
      <ScrollView style={s.body} keyboardShouldPersistTaps="handled">
        ...
        <TextInput
          style={s.input}
          value={draft.brand ?? ''}
          placeholder="e.g. Chipotle, Home cooking"
          returnKeyType="next"
          onChangeText={(v) => void updateDraft(draftId, { brand: v })}
        />
        ...
        <TextInput
          ...
          blurOnSubmit
          onChangeText={(v) => void updateDraft(draftId, { note: v })}
        />
      </ScrollView>

      <SafeAreaView edges={['bottom']} style={s.footerWrap}>
        <TouchableOpacity style={s.saveBtn} onPress={() => navigation.navigate('Tabs')}>
...
const s = StyleSheet.create({
  ...
  fieldLabel: { fontSize: 11, fontWeight: '700', color: '#666', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 6, marginTop: 14 },
  input: { padding: 11, borderRadius: 9, borderWidth: 1, borderColor: '#e6e3d9', backgroundColor: '#fff', fontSize: 15, color: '#111' },
  ...
  dateBtn: {
    ...
    padding: 11, borderRadius: 9, borderWidth: 1, borderColor: '#e6e3d9',
    backgroundColor: '#fff',
  },
